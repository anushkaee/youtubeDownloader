...
import com.github.youtube.downloader.cleaner.FileCleaner;
...
public class DownloadService {
	...
	//	downloader.addCipherFunctionEquivalent("some regex for js function", new ReverseFunction());
		...
		fileCleaner.addToDeleteQue(System.currentTimeMillis(), youtubeVideoId);
		return initStatus;
	}
	
	public InitStatus downloadMp3(String youtubeVideoId) throws YoutubeException, IOException {
	...
	//	downloader.addCipherFunctionEquivalent("some regex for js function", new ReverseFunction());
		...
		youtubeDownloadListener.setFileName(video.details().title());
		youtubeDownloadListener.setVid(youtubeVideoId);
		youtubeDownloadListener.setExt("mp3");
		youtubeDownloadListener.setBaseURl(baseUrl);
		video.downloadAsync(audioFormats.get(0), outputDir,youtubeDownloadListener);
		fileMap.put(youtubeVideoId, youtubeDownloadListener);
	...
	public static Map<String, YoutubeDownloadListener> getFileMap() {
		return fileMap;
	}
}
